...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[5]
    ...
    // layer 9 - Adapted for growave green color palette ********************************************************************
    ...
    * buf[7]
    ...
    // Enhanced green color output with growave palette
    vec3 baseColor = vec3(buf[0].x, buf[0].y, buf[0].z);
    
    // Growave green tint (HSL: 142 100% 64%)
    vec3 growaveGreen = vec3(0.29, 1.0, 0.64); // RGB approximation
    vec3 growaveAccent = vec3(0.0, 0.2, 0.1); // Dark green accent
    
    // Mix shader output with growave colors
    vec3 finalColor = mix(baseColor * 0.3, growaveGreen * 0.8, smoothstep(0.2, 0.8, length(baseColor)));
    finalColor = mix(finalColor, growaveAccent, smoothstep(0.8, 1.0, length(baseColor)));
    ...
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-24 sm:gap-8 sm:pt-32 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-growave-green/20 bg-growave-green/5 px-3 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-xs font-light tracking-tight text-growave-green">Clínica com Rumo</span>
        </div>

        <h1 ref={headerRef} className={`max-w-2xl text-left font-extralight leading-[1.05] tracking-tight text-white ${isMobile ? 'text-4xl' : 'text-5xl sm:text-6xl md:text-7xl'}`}>
          Transforme sua Clínica com o método{' '}
          <span className="gradient-text">Clínica com Rumo</span>
        </h1>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button className="text-sm py-4 px-5 bg-growave-green text-black hover:bg-growave-green-light animate-pulse-soft rounded-2xl">
          ...
          <a
            href="#sobre"
            className="rounded-2xl border border-growave-green/20 px-5 py-3 text-sm font-light tracking-tight text-white/80 hover:bg-growave-green/10 transition-colors focus:outline-none focus:ring-2 focus:ring-growave-green/30 duration-300"
          >
            Saber mais
          </a>
        </div>
